...
class AiApplication(Application):
    ...
    def __init__(self, prefix='', cfg_path='config.json'):
            ...
            if os.path.exists(controller_dir):
                for controller_name in os.listdir(controller_dir):
                    if 'controller' in controller_name:
                        ...
                        routes.append(
                            ('/%s%s/?.*' % (prefix, controller_name), clazz)
                        )
                        route_names.append(controller_name)
            logging.debug('Init App with routes %s', route_names)
            logging.debug('Init App with routes %s', static_folder)
            # session
    ...
    def start_task(self, name):
        ...
        try:
            ...
            pass
        # --------
        p = multiprocessing.Process(target=AiApplication.start_task_process, args=(name,))
        p.start()
